...
app.post('/api/public/requests',publicSubmitLimiter,(req,res)=>{
  ...
  if([data.firstName,data.lastName,data.document,data.mediaName,data.roleTitle,data.coverage,data.email,data.phone].some(v=>!v)){
...
app.post('/api/admin/events',auth,(req,res)=>{
  ...
  audit(req,req.admin.id,'EVENT_CREATED','event',result.lastInsertRowid,{title,eventDate,venue});
...
app.patch('/api/admin/events/:id',auth,(req,res)=>{
  ...
  audit(req,req.admin.id,'EVENT_UPDATED','event',id,{fromStatus:e.status,toStatus:status});
  res.json({ok:true});
});
...
app.get('/health',(req,res)=>res.json({ok:true,version:'6.9.0'}));
app.listen(PORT,()=>console.log(`IDV Acreditaciones v6.9.0: http://localhost:${PORT}`));
