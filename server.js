...
const adminEmail = (process.env.ADMIN_EMAIL || process.env['CORREO ELECTRÓNICO DEL ADMINISTRADOR'] || 'prensa@idv.ec').toLowerCase();
const adminPassword = process.env.ADMIN_PASSWORD || process.env.ADMIN_CONTRASEÑA || 'IDV2026!';
...
['index.html','prensa.html','fotografos.html','admin.html','estado.html','verificar.html','styles.css'].forEach(file=>{
...
app.post('/api/admin/login',(req,res)=>{
  ...
  if(!a || !bcrypt.compareSync(password,a.password_hash)) return res.status(401).json({error:'Correo o contraseña incorrectos'});
...
app.post('/api/public/requests',(req,res)=>{
  ...
  const data={
    firstName:clean(req.body.firstName), lastName:clean(req.body.lastName), document:clean(req.body.document),
    mediaName:clean(req.body.mediaName), roleTitle:clean(req.body.roleTitle), coverage:clean(req.body.coverage),
    email:clean(req.body.email).toLowerCase(), phone:clean(req.body.phone), notes:clean(req.body.notes)
  };
  if(Object.values(data).slice(0,8).some(v=>!v)) return res.status(400).json({error:'Completa todos los campos obligatorios.'});
  const code=uniqueCode();
  try{
    db.prepare(`INSERT INTO requests (event_id,public_code,category,first_name,last_name,document,media_name,role_title,coverage,email,phone,notes) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)`)
...
app.get('/api/public/verify/:token',(req,res)=>{
  const r=db.prepare(`SELECT r.id,r.public_code,r.category,r.first_name,r.last_name,r.media_name,r.role_title,r.status,r.assignment,r.attendance_status,r.attendance_at,e.id event_id,e.title,e.event_date,e.venue FROM requests r JOIN events e ON e.id=r.event_id WHERE r.verify_token=?`).get(req.params.token);
...
app.post('/api/admin/events',auth,(req,res)=>{
  const title=clean(req.body.title), opponent=clean(req.body.opponent), eventDate=clean(req.body.eventDate), venue=clean(req.body.venue), deadline=clean(req.body.deadline);
  if(!title || !eventDate) return res.status(400).json({error:'Título y fecha son obligatorios.'});
  const result=db.prepare(`INSERT INTO events(title,opponent,event_date,venue,deadline,status) VALUES(?,?,?,?,?,'draft')`).run(title,opponent,eventDate,venue,deadline);
...
app.patch('/api/admin/events/:id',auth,(req,res)=>{
  ...
  db.prepare(`UPDATE events SET title=?,opponent=?,event_date=?,venue=?,deadline=?,status=? WHERE id=?`).run(clean(req.body.title ?? e.title),clean(req.body.opponent ?? e.opponent),clean(req.body.eventDate ?? e.event_date),clean(req.body.venue ?? e.venue),clean(req.body.deadline ?? e.deadline),status,id);
  res.json({ok:true});
});
...
app.get('/api/admin/requests',auth,(req,res)=>{
  ...
  let sql=`SELECT r.*,e.title event_title FROM requests r JOIN events e ON e.id=r.event_id WHERE 1=1`; const params=[];
...
app.patch('/api/admin/requests/:id',auth,(req,res)=>{
  const id=Number(req.params.id); const r=db.prepare('SELECT * FROM requests WHERE id=?').get(id);
  if(!r) return res.status(404).json({error:'Solicitud no encontrada'});
  const status=req.body.status ?? r.status;
  if(!['pending','approved','rejected','delivered'].includes(status)) return res.status(400).json({error:'Estado inválido'});
  ...
  if(!['present','absent'].includes(attendanceStatus)) attendanceStatus = r.attendance_status || 'absent';
  ...
  if(req.body.attendanceStatus==='present') attendanceAt = attendanceAt || new Date().toISOString();
  if(req.body.attendanceStatus==='absent') attendanceAt = null;
  ...
  db.prepare(`UPDATE requests SET status=?,assignment=?,admin_notes=?,verify_token=?,reviewed_at=?,delivered_at=?,attendance_status=?,attendance_at=? WHERE id=?`).run(status,assignment,adminNotes,token,reviewedAt,deliveredAt,attendanceStatus,attendanceAt,id);
  res.json({ok:true});
});
...
app.post('/api/admin/vests/checkout/:requestId',auth,(req,res)=>{
  ...
  const vestNumber=clean(req.body.vestNumber); const signature=clean(req.body.signature); const retained=req.body.credentialRetained ? 1 : 0;
...
app.post('/api/admin/vests/return/:requestId',auth,(req,res)=>{
  ...
  const vestNumber=clean(req.body.vestNumber); const signature=clean(req.body.signature);
...
app.get('/api/admin/events/:id/security.csv',auth,(req,res)=>{
  ...
  const rows=db.prepare(`SELECT category,first_name,last_name,document,media_name,role_title,assignment,status,attendance_status FROM requests WHERE event_id=? AND status IN ('approved','delivered') ORDER BY category,media_name,last_name,first_name`).all(eventId);
  sendCsv(res, `seguridad-${eventId}.csv`, ['Tipo','Nombres','Apellidos','Documento','Medio','Función','Asignación','Estado','Asistencia'], rows.map(r=>[
    r.category==='press'?'Prensa escrita':'Fotógrafo', r.first_name, r.last_name, r.document, r.media_name, r.role_title, r.assignment||'', r.status, r.attendance_status==='present'?'Presente':'No registrado'
  ]));
});
...
app.get('/api/admin/events/:id/attendance.csv',auth,(req,res)=>{
  const eventId=Number(req.params.id); const event=db.prepare('SELECT * FROM events WHERE id=?').get(eventId);
  if(!event) return res.status(404).json({error:'Evento no encontrado'});
  const rows=db.prepare(`SELECT category,first_name,last_name,document,media_name,role_title,assignment,status,attendance_status,attendance_at FROM requests WHERE event_id=? AND status IN ('approved','delivered') ORDER BY category,media_name,last_name,first_name`).all(eventId);
  sendCsv(res, `asistencia-${eventId}.csv`, ['Tipo','Nombres','Apellidos','Documento','Medio','Función','Asignación','Estado credencial','Asistencia','Fecha asistencia'], rows.map(r=>[
    r.category==='press'?'Prensa escrita':'Fotógrafo', r.first_name, r.last_name, r.document, r.media_name, r.role_title, r.assignment||'', r.status, r.attendance_status==='present'?'Presente':'No registrado', r.attendance_at||''
  ]));
});
...
async function sendSecurityWorkbook(req,res,mode){
  try{
    ...
    const rows=db.prepare(`
      SELECT category,first_name,last_name,document,media_name,role_title,
             assignment,status,attendance_status,attendance_at
      FROM requests
      WHERE event_id=? AND status IN ('approved','delivered')
      ORDER BY category,media_name,last_name,first_name
    `).all(eventId);

    const normalized=rows.map(r=>({
      type:r.category==='press'?'Prensa escrita':'Fotógrafo',
    ...
    const press=normalized.filter(r=>r.type==='Prensa escrita').length;
    const photo=normalized.filter(r=>r.type==='Fotógrafo').length;
    ...
    if(mode==='attendance'){
      ...
      addIdvSheet(workbook,{
        ...
        rows:normalized,
        columns:baseColumns,
        summary:[`Acreditados: ${total}`,`Prensa: ${press}`,`Fotógrafos: ${photo}`,`Presentes: ${present}`]
      });
      addIdvSheet(workbook,{
        name:'Prensa escrita',
        title:`PRENSA ESCRITA · ${event.title}`,
        subtitle:eventMeta,
        rows:normalized.filter(r=>r.type==='Prensa escrita'),
        columns:baseColumns,
        summary:[`Prensa acreditada: ${press}`]
      });
      addIdvSheet(workbook,{
        name:'Fotógrafos',
        title:`FOTÓGRAFOS · ${event.title}`,
        subtitle:eventMeta,
        rows:normalized.filter(r=>r.type==='Fotógrafo'),
        columns:baseColumns,
...
app.get('/health',(req,res)=>res.json({ok:true,version:'6.5'}));
app.listen(PORT,()=>console.log(`IDV Acreditaciones v6.5: http://localhost:${PORT}`));
