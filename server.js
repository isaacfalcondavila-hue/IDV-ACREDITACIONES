...
app.get('/api/admin/vests',auth,(req,res)=>{
  const eventId=Number(req.query.eventId||0);
  let sql=`SELECT vl.*,r.first_name,r.last_name,r.media_name,r.public_code,e.title event_title
           FROM vest_loans vl
           JOIN requests r ON r.id=vl.request_id
           JOIN events e ON e.id=vl.event_id WHERE 1=1`;
  const params=[];
  if(eventId){sql+=' AND vl.event_id=?';params.push(eventId)}
  sql+=' ORDER BY CASE WHEN vl.status="out" THEN 0 ELSE 1 END, CAST(vl.vest_number AS INTEGER), vl.vest_number';
  res.json(db.prepare(sql).all(...params));
});
...
app.listen(PORT,()=>console.log(`IDV Acreditaciones v4: http://localhost:${PORT}`));
