...
## v7.0 — Identidad visual institucional IDV

Actualización exclusivamente visual. No se modificó la lógica de acreditaciones, QR,
correo, seguridad, asistencia, chalecos ni reportes.

### Dirección
La estructura pública toma como referencia la claridad de una página institucional de prensa:
- cabecera del partido;
- competición;
- explicación breve del proceso;
- aviso importante de que el registro no implica acreditación automática;
- selección del tipo de cobertura;
- formulario.

Todo se mantiene dentro de la identidad IDV.

### Textura
Se añadió `assets/idv-texture.jpg`, usando EXACTAMENTE la textura suministrada.
No se modifica su geometría ni se genera una textura nueva.

La textura se oscurece únicamente mediante CSS:
- opacidad muy baja;
- overlay negro / azul oscuro;
- sin fondos llenos de luz;
- sin escudos gigantes ni textos decorativos de fondo.

### Diseño
- Negro y azul oscuro como base.
- Azul fuerte reservado para acciones importantes.
- Tarjetas sobrias y semitransparentes.
- Bordes azules muy sutiles.
- Header institucional con los tres escudos existentes.
- Formularios más editoriales, con explicación previa del proceso.
- Panel administrativo actualizado visualmente sin cambiar su estructura.


## v7.0.1 — Fix visual de textura

Se corrigió la implementación del fondo institucional.

La textura original `assets/idv-texture.jpg` ahora se aplica directamente al `body`,
con una capa oscura encima. Antes estaba en un pseudo-elemento con z-index negativo,
lo que podía dejarla completamente oculta detrás del fondo sólido.

También se reforzó ligeramente el estado `No hay acreditaciones abiertas` para que la
identidad visual se perciba incluso cuando no existe un evento abierto.

No se cambió ninguna lógica funcional.


## v7.1 — Visual institucional completo

La actualización visual ahora aplica a TODA la plataforma, no únicamente cuando existe
una acreditación abierta.

Incluye:
- Home sin evento activo con jerarquía editorial institucional.
- Home con evento activo.
- Flujo previo de Libertadores.
- Formularios de medios y fotógrafos.
- Estados cerrados de formularios.
- Login de Administración.
- Panel administrativo completo.
- Consulta de estado.
- Verificación QR.
- Header y footer visualmente coherentes.
- Textura IDV original, oscurecida únicamente mediante CSS.

La lógica funcional permanece intacta.


## v7.1.1 — Creación de eventos

- Se eliminó el texto de ejemplo `IDV vs Flamengo`.
- Se eliminó el texto de ejemplo `Estadio Olímpico Atahualpa`.
- Título y estadio quedan completamente vacíos al crear un nuevo evento.
- La hora del partido ahora se selecciona explícitamente con hora, minutos y `AM / PM`.
- La fecha límite usa el mismo formato.
- Internamente se convierte al formato que ya utiliza el backend, por lo que no cambia la lógica.
- En Administración y páginas públicas la hora se muestra como `7:30 PM`, `10:00 AM`, etc.


## v7.2 — Solicitudes de entrevista

Se añadió un segundo flujo de Prensa independiente de las acreditaciones de partido.

### Disponible todo el año
La portada siempre muestra `Solicitud de entrevista`, haya o no un partido con acreditaciones abiertas.

### Formulario
Basado en el formato histórico de IDV:
- Entrevistado
- Temas a tratar
- Medio
- Programa
...
- Tiempo máximo
- Día
- Vía: Radial / Presencial / Zoom

Se añadieron `Correo` y `Teléfono` para poder comunicar la decisión al solicitante.
...
La solicitud de entrevista no depende de ningún evento ni partido.

### Correos
Al enviar el formulario:
- se guarda inmediatamente en la base de datos;
- llega un correo interno con asunto `Solicitud de entrevista`;
- el destinatario es `ADMIN_EMAIL`.

Opcionalmente puede definirse una variable:
`INTERVIEW_NOTIFY_EMAIL`
si se desea enviar estas alertas a otro correo distinto al administrador.

Al aprobar o rechazar:
- el periodista recibe automáticamente un correo con la decisión;
- se puede agregar un mensaje desde el panel.

El envío utiliza la misma integración de Resend que las acreditaciones.


## v7.2.1 — Correo de prueba para solicitudes de entrevista

Durante la fase de prueba, las nuevas solicitudes de entrevista se notifican por defecto a:

`isaac.falcon.davila@gmail.com`

Este correo coincide con la cuenta utilizada para las pruebas de Resend.

No se utiliza `ADMIN_EMAIL` para estas notificaciones.

Si más adelante se crea la variable Railway `INTERVIEW_NOTIFY_EMAIL`, esa variable tendrá
prioridad y permitirá cambiar el destinatario sin modificar el código.


## v7.2.2 — Reglas de entrevistas

- El formulario de entrevistas queda identificado como exclusivo para jugadores del primer equipo.
- Toda entrevista debe solicitarse con mínimo 72 horas de anticipación.
- La regla de 72 horas se valida tanto en navegador como en backend.
- Para el cálculo se usa horario de Ecuador continental (UTC-5).
- Al aprobar una solicitud, el mensaje enviado al periodista es fijo:

`Mensaje de Prensa IDV:
Solicitud aprobada. Comunicate con el jefe de prensa Juan Gonzales y su numero +593 994476462`

- El panel ya no solicita redactar un mensaje al aprobar; únicamente confirma la acción.
- En un rechazo sí se mantiene la opción de escribir el motivo o mensaje correspondiente.
