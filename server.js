...
app.post('/api/public/requests',publicSubmitLimiter,(req,res)=>{
  ...
  try{
    db.prepare(`INSERT INTO requests (
      event_id,public_code,category,accreditation_type,first_name,last_name,document,
      media_name,role_title,coverage,email,phone,notes
    ) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)`)
      .run(
        event.id,code,category,accreditationType,data.firstName,data.lastName,data.document,
...
app.get('/api/admin/events/:id/security.csv',auth,(req,res)=>{
  ...
  const rows=db.prepare(`SELECT category,accreditation_type,first_name,last_name,document,media_name,role_title,assignment,status,attendance_status FROM requests WHERE event_id=? AND status IN ('approved','delivered') ORDER BY LOWER(media_name), category, LOWER(last_name), LOWER(first_name)`).all(eventId);
  sendCsv(res, `seguridad-${eventId}.csv`, ['Tipo','Nombres','Apellidos','Documento','Medio','Función','Restricción','Asignación','Estado','Asistencia'], rows.map(r=>[
    accreditationTypeLabel(r.accreditation_type,r.category), r.first_name, r.last_name, r.document,
...
app.get('/api/admin/events/:id/attendance.csv',auth,(req,res)=>{
  const eventId=Number(req.params.id); const event=db.prepare('SELECT * FROM events WHERE id=?').get(eventId);
  if(!event) return res.status(404).json({error:'Evento no encontrado'});
  const rows=db.prepare(`SELECT category,accreditation_type,first_name,last_name,document,media_name,role_title,assignment,status,attendance_status,attendance_at FROM requests WHERE event_id=? AND status IN ('approved','delivered') ORDER BY LOWER(media_name), category, LOWER(last_name), LOWER(first_name)`).all(eventId);
  sendCsv(res, `asistencia-${eventId}.csv`, ['Tipo','Nombres','Apellidos','Documento','Medio','Función','Restricción','Asignación','Estado credencial','Asistencia','Fecha asistencia'], rows.map(r=>[
    accreditationTypeLabel(r.accreditation_type,r.category), r.first_name, r.last_name, r.document,
    r.media_name, r.role_title, accreditationRestriction(r.accreditation_type), r.assignment||'',
...
async function sendSecurityWorkbook(req,res,mode){
  try{
    ...
    const rows=db.prepare(`
      SELECT category,accreditation_type,first_name,last_name,document,media_name,role_title,
...
app.get('/health',(req,res)=>res.json({ok:true,version:'6.11.0'}));
app.listen(PORT,()=>console.log(`IDV Acreditaciones v6.11.0: http://localhost:${PORT}`));
