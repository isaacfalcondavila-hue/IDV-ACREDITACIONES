...
const SMTP_FROM_NAME = cleanEnv(process.env.SMTP_FROM_NAME || 'Acreditaciones IDV');

function cleanEnv(v){ return String(v ?? '').trim(); }

const mailConfigured = Boolean(SMTP_HOST && SMTP_PORT && SMTP_USER && SMTP_PASS && SMTP_FROM_EMAIL);
...
async function sendApprovalEmail(requestRow){
  if(!mailConfigured){
    return {
      sent:false,
      warning:'Correo no configurado. Agrega SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_PASS y SMTP_FROM_EMAIL en Railway.'
    };
  }
  ...
  const typeLabel = requestRow.category === 'photo' ? 'Fotógrafo' : 'Prensa escrita';
            ...
            <tr>
              <td style="padding:30px">
                ...
                <p style="color:#d8e2f6;font-size:15px;line-height:1.6">
                  Presenta este código QR el día del partido. También puedes abrir tu acreditación desde el botón inferior.
                </p>

                <div style="text-align:center;margin:24px 0">
                  <img src="cid:idv-accreditation-qr" width="220" height="220" alt="QR de acreditación" style="display:inline-block;background:#ffffff;padding:12px;border-radius:18px">
                </div>
...
Independiente del Valle · Acreditaciones`,
    html,
    attachments: [{
      filename:'acreditacion-idv-qr.png',
      content:qrBuffer,
      cid:'idv-accreditation-qr',
...
app.get('/api/admin/events/:id/security.csv',auth,(req,res)=>{
  ...
  const rows=db.prepare(`SELECT category,first_name,last_name,document,media_name,role_title,assignment,status,attendance_status FROM requests WHERE event_id=? AND status IN ('approved','delivered') ORDER BY LOWER(media_name), category, LOWER(last_name), LOWER(first_name)`).all(eventId);
...
app.get('/api/admin/events/:id/attendance.csv',auth,(req,res)=>{
  const eventId=Number(req.params.id); const event=db.prepare('SELECT * FROM events WHERE id=?').get(eventId);
  if(!event) return res.status(404).json({error:'Evento no encontrado'});
  const rows=db.prepare(`SELECT category,first_name,last_name,document,media_name,role_title,assignment,status,attendance_status,attendance_at FROM requests WHERE event_id=? AND status IN ('approved','delivered') ORDER BY LOWER(media_name), category, LOWER(last_name), LOWER(first_name)`).all(eventId);
...
app.get('/health',(req,res)=>res.json({ok:true,version:'6.6'}));
app.listen(PORT,()=>console.log(`IDV Acreditaciones v6.6: http://localhost:${PORT}`));
