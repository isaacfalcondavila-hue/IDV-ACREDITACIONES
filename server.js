      ...
      <tr><td align="center">
        <table role="presentation" width="620" cellspacing="0" cellpadding="0" style="max-width:620px;width:100%;background:#0d1119;border:1px solid #263248;border-radius:20px;overflow:hidden">
          ...
          <tr><td style="padding:28px">
            ...
            ${row.admin_notes ? `<div style="margin-top:18px;padding:14px 16px;background:#0b1628;border:1px solid #315b9b;border-radius:12px;color:#d8e2f6"><strong>Mensaje de Prensa IDV:</strong><br>${escapeHtml(row.admin_notes)}</div>` : ''}
...
Vía: ${interviewModeLabel(row.interview_mode)}
${row.admin_notes ? `Mensaje de Prensa IDV: ${row.admin_notes}` : ''}
...
const INTERVIEW_NOTIFY_EMAIL = cleanEnv(process.env.INTERVIEW_NOTIFY_EMAIL || adminEmail);
...
app.patch('/api/admin/interviews/:id',auth,async(req,res)=>{
  ...
  const adminNotes=boundedText(req.body.adminNotes ?? current.admin_notes,1000);
...
app.get('/health',(req,res)=>res.json({ok:true,version:'7.2.0'}));
app.listen(PORT,()=>console.log(`IDV Acreditaciones v7.2.0: http://localhost:${PORT}`));
